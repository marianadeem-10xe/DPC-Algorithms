...
class DPC:
    ...
    def detect_dead_pixels(self):
        ...
        gh_l, gv_l, diff_l = [],[], []
        for i in range(img_padded.shape[0]-4):        #column
            for j in range(img_padded.shape[1]-4):     #row
                to_be_tested_pv = img_padded[i + 2, j + 2]
                d1              = img_padded[i, j]
                top             = img_padded[i, j + 2]
                d2              = img_padded[i, j + 4]
                left            = img_padded[i + 2, j]
                right           = img_padded[i + 2, j + 4]
                d3              = img_padded[i + 4, j]
                bottom          = img_padded[i + 4, j + 2]
                d4              = img_padded[i + 4, j + 4]
