...
for raw_path in paths:    
    ...
    out_img_path    = main_path +"Yongji/corrected images/DPC_yongji_imp_3th_min, max, 20_" + raw_filename +".png"
    out_mask_path   = main_path +"Yongji/corrected masks/corrected_mask_yongji_imp_3th_min, max, 20_" + raw_filename +".raw"
    ...
    if run_DPC:
        ...
        # plt.imsave(str(Path(raw_path).parent.parent)+ "/Input images/" + raw_filename + "_Input_image_.png", save_img)

        print(def_img.shape)
        print(np.amax(def_img), np.amin(def_img))
        
        dpc        = yongji_improved.DPC(def_img, size, 20, np.amin(def_img), np.amax(def_img)) 
